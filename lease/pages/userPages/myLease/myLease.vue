		...
		<view class="card2" v-for='(item,index) in list' :key="index">
			<view class="card">
				...
				<view class="title">
					...
					<view>租赁时间：{{item.start_time==null?'暂无':item.start_time}}</view>
					<view>
						<text>到期时间：{{item.end_time==null?'暂无':item.end_time}}</text>
						<navigator :url="'../yuSuccess/yuSuccess?id='+item.id" hover-class="none">详情</navigator>
					</view>
				</view>
			</view>
			<view class="flexBox btn">
				<navigator :url="'../yuYue3/yuYue3?id='+item.dian_id+'&lid='+item.id" hover-class="none">预约更换</navigator>
				<navigator :url="'../tMoney/tMoney?id='+item.id+'&lid='+item.id" hover-class="none">退押金</navigator>
			</view>
			<image :src="item.status==2||item.status==3?'../../../static/image/tuikuan.png':''" mode="" class="tui"></image>
		</view>
	</view>
</template>
